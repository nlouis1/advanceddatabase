...
def pet_to_dict(pet):
    return {
        ...
        "owner": pet.owner,
        "kind_id": pet.kind.id,
        "kind_name": pet.kind.kind_name,
        "kind_food": pet.kind.food,
        "kind_noise": pet.kind.noise
    }

def create_pet(data):
    required_keys = ["name", "age", "kind_id", "owner"]
...
def get_pet_by_id(id):
    return Pet.select().join(Kind).where(Pet.id == id).first()
...
def get_kind_by_name(name):
    return Kind.get_or_none(Kind.name == name)
